...
void TreeScene::reset() {
    ...
    for (auto item : m_trashItems) { if (item) { removeItem(item); delete item; } }
    m_trashItems.clear();
    update();
}
...
void TreeScene::animSearchPath(int targetVal, std::function<void(TreeNode*, TreeNode*, bool)> onFinished) {
    ...
    if (!pathPoints.isEmpty()) m_probeHalo->setPos(pathPoints.first());
    m_probeHalo->setVisible(true); m_probeHalo->setOpacity(1.0);
    ...
    for (int i = 0; i < pathPoints.size() - 1; ++i) {
        ...
        move->setDuration(500); move->setStartValue(pathPoints[i]); move->setEndValue(pathPoints[i + 1]);
...
void TreeScene::traverseAnimated(int type) {
    ...
    if (first && first->circle) m_probeHalo->setPos(first->circle->pos() - QPointF(5, 5));
    else m_probeHalo->setPos(root->circle->pos() - QPointF(5, 5));

    QSequentialAnimationGroup* group = new QSequentialAnimationGroup(this);
    QString str = typeName;
    buildTraversalAnim(group, root, type, str);
...
void TreeScene::addVisitAnim(QSequentialAnimationGroup* group, TreeNode* node, QString& currentStr) {
    ...
    if (!currentStr.endsWith(": ")) currentStr += " -> ";
    ...
    connect(visit, &QVariantAnimation::stateChanged, this, [this, displayStr](QAbstractAnimation::State ns, QAbstractAnimation::State) {
        if (ns == QAbstractAnimation::Running && m_resultText) m_resultText->setText(displayStr);
        });
